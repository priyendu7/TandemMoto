...
- nothing about playback is stored, but while you're connected each play, pause, skip or seek sends your partner's phone which song is playing (its fingerprint, not its title), whether it's playing, and where it is, so both phones play the same thing,
